...
CREATE TABLE `INSTT` (
	...
	`REPRSNT_TLPHON` VARCHAR(127) NULL     COMMENT '대표 전화', -- 대표 전화
...
CREATE TABLE `DGNSS_DBMS` (
	`INSTT_CODE`    VARCHAR(20)  NOT NULL COMMENT '기관 코드', -- 기관 코드
	`DGNSS_DBMS_ID` VARCHAR(20)  NOT NULL COMMENT '진단 DBMS 아이디', -- 진단 DBMS 아이디
	`DGNSS_DBMS_NM` VARCHAR(255) NULL     COMMENT '진단 DBMS 명', -- 진단 DBMS 명
	`DBMS_KND`      VARCHAR(20)  NULL     COMMENT 'DBMS 종류', -- DBMS 종류
	`DBMS_VER`      VARCHAR(20)  NULL     COMMENT 'DBMS 버전', -- DBMS 버전
...
ALTER TABLE `DGNSS_DBMS`
	ADD CONSTRAINT `PK_DGNSS_DBMS` -- 진단 DBMS 정보 기본키
		PRIMARY KEY (
			`INSTT_CODE`,    -- 기관 코드
			`DGNSS_DBMS_ID`  -- 진단 DBMS 아이디
		);
...
CREATE TABLE `DGNSS_TABLES` (
	...
	`DGNSS_DBMS_ID`  VARCHAR(20)  NOT NULL COMMENT '진단 DBMS 아이디', -- 진단 DBMS 아이디
	`SCHEMA_NM`      VARCHAR(255) NULL     COMMENT '스키마 명', -- 스키마 명
...
CREATE TABLE `AUTHORITY` (
	...
	`AUTHORY_ID` VARCHAR(255) NULL     COMMENT '권한자 ID' -- 권한자 ID
)
collate = `utf8_general_ci` ENGINE = InnoDB 
COMMENT '이용자권한';
...
-- 이용자
CREATE TABLE `USER` (
	`ENGN_CODE`        VARCHAR(20)  NOT NULL COMMENT '기관코드', -- 기관코드
	`USER_ID`          VARCHAR(255) NOT NULL COMMENT '이용자 ID', -- 이용자 ID
...
-- 공통코드
ALTER TABLE `DQ_CMMN_CODE`
	ADD CONSTRAINT `PK_DQ_CMMN_CODE` -- 공통코드 기본키
		PRIMARY KEY (
			`GROUP_CODE` -- 그룹 코드
		);
...
CREATE TABLE `MENU_MANAGE` (
	...
	`MENU__URL`     VARCHAR(600) NULL     COMMENT '메뉴 URL', -- 메뉴 URL
	`USE_AT`        VARCHAR(1)   NULL     COMMENT '사용 여부', -- 사용 여부
...
-- 공통코드상세
ALTER TABLE `DQ_CMMN_CODE_DETAIL`
	ADD CONSTRAINT `PK_DQ_CMMN_CODE_DETAIL` -- 공통코드상세 기본키
		PRIMARY KEY (
			`GROUP_CODE`,      -- 그룹 코드
			`CMMN_CODE`,       -- 공통 코드
...
-- 진단 정보
ALTER TABLE `DGNSS_TABLES`
	ADD CONSTRAINT `FK_DGNSS_DBMS_TO_DGNSS_TABLES` -- 진단 DBMS 정보 -> 진단 정보
		FOREIGN KEY (
			`INSTT_CODE`,    -- 기관 코드
			`DGNSS_DBMS_ID`  -- 진단 DBMS 아이디
		)
		REFERENCES `DGNSS_DBMS` ( -- 진단 DBMS 정보
			`INSTT_CODE`,    -- 기관 코드
			`DGNSS_DBMS_ID`  -- 진단 DBMS 아이디
		);
...
-- 빈도 분석 결과
ALTER TABLE `FRQ_ANALS`
	ADD CONSTRAINT `FK_DGNSS_TABLES_TO_FRQ_ANALS` -- 진단 정보 -> 빈도 분석 결과
		FOREIGN KEY (
			`INSTT_CODE`,    -- 기관 코드
			`DGNSS_INFO_ID`  -- 진단 정보 아이디
		)
...
-- 공통코드상세
ALTER TABLE `DQ_CMMN_CODE_DETAIL`
	ADD CONSTRAINT `FK_DQ_CMMN_CODE_TO_DQ_CMMN_CODE_DETAIL` -- 공통코드 -> 공통코드상세
		FOREIGN KEY (
			`GROUP_CODE` -- 그룹 코드
		)
